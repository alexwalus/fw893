...
isCelsius  = grep(pattern = "C|Celsius|ce|c", x=classData2$date);
tempvalues = gsub(pattern = "[^+-0123456789]", replacement="", 
                  x=classData2$temp);
tempvalues2 = as.numeric(tempvalues);    # make the vector numeric (this is NOT automatic)
classData2$tempvalues = tempvalues2; # assign vector to a new column